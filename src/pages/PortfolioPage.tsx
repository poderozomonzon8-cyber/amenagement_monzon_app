  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Browse our portfolio of 250+ construction, renovation, and landscaping projects."
        />
      </Helmet>
            ...
            <div className="flex flex-wrap gap-2 mb-10" role="group" aria-label="Filter portfolio">
              {FILTERS.map((f) => (
                <button
                  ...
                  onClick={() => setFilter(f)}
                  className={[
                    "px-4 py-2 text-xs font-mono font-medium rounded-xl border transition-all duration-200 focus:outline-none cursor-pointer",
                    f === filter
                ...
                {filtered.map((item, i) => (
                  ...
                  >
                    ...
                    <div className="absolute top-4 left-4">
                      <span className="glass px-3 py-1.5 text-[10px] font-mono text-gold/90 rounded-full">
                        {item.category}
                      </span>
                    </div>
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-5 translate-y-2 group-hover:translate-y-0 transition-transform duration-400">
                      <span className="font-mono text-[10px] text-gold/70 uppercase tracking-wider">
                        {item.year}
                      </span>
                      <h3 className="font-headline font-bold text-lg text-warm-white mt-1 leading-tight">
                        {item.title}
                      </h3>
