...
const FALLBACK_SERVICES = [
  { id: "s1", name: "Exterior Maintenance Package", description: "Complete exterior upkeep including pressure washing, gutter cleaning, window cleaning, and façade inspection.", category: "exterior", price: "From $199", priceType: "monthly", subscriptionAvailable: "yes", features: '["Pressure washing","Gutter cleaning","Window cleaning","Façde inspection","Monthly report"]', status: "active", popular: "yes" },
...
const SUBSCRIPTIONS = [
  ...
  { id: "sub2", name: "Quarterly Plan", desc: "Commit to 3 months and save 10% on all services.", price: "From $405/qtr", badge: "Save 10%", color: "border-gold/40" },
  ...
  { id: "sub4", name: "Custom Plan", desc: "Tailored service bundles for commercial properties.", price: "On Request", badge: "Enterprise", color: "border-charcoal/20" },
];
...
export default function MaintenancePage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        console.warn("Maintenance load error:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Residential and commercial maintenance service packages and subscriptions."
        />
      </Helmet>
      ...
      <PageShell>
        <PageHero
          ...
          subtitle="Recurring and one-time maintenance plans for residential and commercial properties. Subscribe and forget — we handle the rest."
        />
          ...
          <div className="max-w-screen-xl mx-auto px-6 md:px-10 flex items-center justify-between flex-wrap gap-4">
            <div className="flex gap-1 p-1 bg-gray-100 rounded-xl">
              <button
                ...
                className={`px-5 py-2 text-sm font-sans font-medium rounded-lg transition-all cursor-pointer ${
                  view === "services"
            ...
            <div className="flex flex-wrap gap-2">
              {CATS.map((c) => (
                <button
                  ...
                  className={[
                    "px-3 py-1.5 text-xs font-mono rounded-xl border transition-all duration-200 cursor-pointer",
                    c.key === cat
            ...
            <div className="max-w-screen-xl mx-auto px-6 md:px-10">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filtered.map((svc) => {
                  const feats = (() => {
                    try {
                      return JSON.parse(svc.features);
                    } catch {
                      return [];
                    }
                  })();
                  ...
                  return (
                    <div
                      key={svc.id}
                      className={`group flex flex-col bg-white rounded-2xl border overflow-hidden transition-all duration-300 hover:shadow-xl hover:shadow-black/5 ${
                        svc.popular === "yes"
                          ? "border-gold/40"
                          : "border-gray-200"
                      }`}
  ...
  {CATEGORY_ICONS[svc.category as keyof typeof CATEGORY_ICONS] ?? (
    <Buildings size={18} />
                          ...
                          <div>
                            <h3 className="font-headline font-bold text-base text-charcoal leading-tight">
                              {svc.name}
                            </h3>
                        ...
                        <div className="mt-auto pt-4 border-t border-gray-100 flex items-center justify-between">
                          <div>
                            <p className="font-headline font-bold text-lg text-charcoal">
                              {svc.price}
                            </p>
                          ...
                          <div className="flex gap-2">
                            {svc.subscriptionAvailable === "yes" && (
                              <button
                                onClick={() =>
                                  handleAdd(`sub-${svc.id}`)
                                }
                                className={`flex items-center gap-1 px-3 py-2 text-[11px] font-sans rounded-xl border transition-all cursor-pointer ${
                                  added.has(`sub-${svc.id}`)
                            ...
                            <button
                              ...
                              className={`flex items-center gap-1 px-3 py-2 text-[11px] font-sans rounded-xl transition-all cursor-pointer ${
                                isAdded
            ...
            <div className="max-w-screen-xl mx-auto px-6 md:px-10">
              <div className="text-center mb-12">
                ...
                <p className="font-sans text-sm text-gray-500 mt-3 max-w-lg mx-auto">
                  All plans include access to our full service catalog. Upgrade
                  or cancel anytime.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                {SUBSCRIPTIONS.map((sub) => (
                  <div
                    key={sub.id}
                    className={`group flex flex-col p-6 rounded-2xl border-2 relative bg-white hover:shadow-xl hover:shadow-black/5 transition-all duration-300 ${sub.color}`}
                  >
                    <span className="inline-block px-2.5 py-0.5 text-[9px] font-mono bg-gold/10 text-gold rounded-full mb-4 w-fit">
                      {sub.badge}
                    </span>

                    <h3 className="font-headline font-bold text-lg text-charcoal mb-2">
                      {sub.name}
                    </h3>

                    <p className="font-sans text-sm text-gray-500 leading-relaxed flex-1 mb-5">
                      {sub.desc}
                    </p>

                    <div className="border-t border-gray-100 pt-4">
                      <p className="font-headline font-bold text-xl text-charcoal mb-4">
                        {sub.price}
                      </p>
              ...
              <div className="mt-12 p-8 bg-charcoal rounded-3xl text-center">
                <p className="font-headline font-bold text-2xl text-warm-white mb-3">
                  Need a custom plan?
                </p>
                <p className="font-sans text-sm text-gray-400 mb-6">
                  Commercial properties, multi-unit buildings, and specialized
                  needs — we'll build a plan around you.
                </p>

                <a
                  href="/contact"
                  className="inline-flex items-center gap-2 px-6 py-3 bg-gold text-charcoal font-sans font-semibold text-sm rounded-xl hover:bg-gold-dark transition-all"
                >
